...
// --- IMPORT รูปภาพ AVATAR (ยังต้องใช้ 2 รูปนี้อยู่ครับ) ---
// ตรวจสอบว่ามีไฟล์ src/assets/male_avatar.png และ female_avatar.png แล้วหรือยัง
...
const PatientListPage = () => {
  ...
  // ข้อมูลบริษัท (ภาษาไทยตามที่ขอ)
  const companies = [
    "ทั้งหมด",
    "บริษัท อินโนวาเทค โซลูชันส์ จำกัด",
    "บริษัท สมาร์ทเซิร์ฟ รีเทล จำกัด",
    "บริษัท พรีเมียร แมนูแฟคเจอริ่ง จำกัด"
  ];

  // ข้อมูลตัวเลือก Dropdown หน่วยงาน
  const units = [
    "ทั้งหมด",
    "ฝ่ายบริหาร", "ฝ่ายบริหารองค์กร", "ฝ่ายวิศวกรรมดิจิทัล", "ฝ่ายข้อมูลและปัญญาประดิษฐ์",
    "ฝ่ายกลยุทธ์", "ฝ่ายบริการ", "ฝ่ายค้าปลีก",
    "ฝ่ายวางแผนการผลิต", "ฝ่ายคุณภาพมาตรฐานโรงงาน", "ฝ่ายบริหารโรงงาน"
  ];

  const departments = [
    "ทั้งหมด",
    "ทรัพยากรบุคคล", "การเงินและบัญชี", "พัฒนาแอปพลิเคชัน", "ทดสอบซอฟต์แวร์", "วิเคราะห์ข้อมูล", "ปัญญาประดิษฐ์",
    "การตลาด", "โปรโมทและโปรโมชั่น", "บริการลูกค้า", "ดูแลหลังการขาย", "คลังสินค้า", "กระจายสินค้า",
    "วางแผนการผลิต", "ดูแลเครื่องจักร", "ตรวจสอบคุณภาพ", "ประกันคุณภาพ", "บัญชีต้นทุน"
  ];

  // --- ดึงข้อมูล (API) & Real-time setup ---
  ...
  useEffect(() => {
    ...
    // Polling ทุก 3 วินาที (Real-time)
  ...
  const getGenderLabelStyle = (gender) => {
    ...
    if (g === 'male' || g === 'ชาย') {
        return { bg: '#e3f2fd', color: '#1565c0' }; // ฟ้า
    } else if (g === 'female' || g === 'หญิง') {
        return { bg: '#fce4ec', color: '#c2185b' }; // ชมพู
    }
    return { bg: '#f3e5f5', color: '#7b1fa2' }; // ม่วง
  };
  ...
  // --- Logic การเรียงลำดับและค้นหา ---
  const processedPatients = [...patients]
    .sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt))
    .filter(p => {
      // หมายเหตุ: ข้อมูลใน Database ฟิลด์ company ต้องตรงกับชื่อภาษาไทยในตัวแปร companies ด้านบน
      // หากใน DB เก็บเป็นภาษาอังกฤษ คุณอาจต้องแก้เงื่อนไขตรงนี้เพิ่มเติมครับ
      ...
      const matchSearch = 
        ...
        (p.hn_no && p.hn_no.includes(searchTerm));
      
      return matchCompany && matchUnit && matchDept && matchSearch;
    });

  const handleViewDetail = (hn) => {
      console.log("Navigating to detail of:", hn); 
      navigate(`/patient-detail/${hn}`); 
  };
          ...
          boxShadow: '0 4px 20px rgba(0,0,0,0.1)' 
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', marginBottom: '20px', gap: '20px' }}>
                ...
                <div style={{ display: 'flex', gap: '15px' }}>
                    ...
                    {/* --- Dropdown บริษัท (แก้ไขแล้ว: ไม่มีโลโก้, ชื่อไทย) --- */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                        ...
                        <div style={{ position: 'relative', width: '250px' }}> 
                            ...
                            <ChevronDown size={14} style={{ position: 'absolute', right: '15px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', color: '#888' }} />
                        </div>
                    </div>
            ...
            <div style={{ 
                display: 'grid', 
                gridTemplateColumns: '1.2fr 3fr 1.5fr 1fr 1.5fr',
                padding: '15px 0', 
                color: '#888', 
                fontWeight: 'bold', 
                fontSize: '14px'
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingRight: '5px' }}>
                {loading ? (
                    <div style={{ textAlign: 'center', marginTop: '20px', color: '#999' }}>กำลังโหลดข้อมูล...</div>
                            ...
                            >
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                    <div style={{ 
                                        ...
                                        overflow: 'hidden', 
                                        border: `2px solid ${genderStyle.bg}`
                                    }}>
                                        {avatarSrc ? (
                                            <img src={avatarSrc} alt={item.gender} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                        ) : (
                                            <User size={20} color={genderStyle.color} />
                                        )}
                                    </div>
                                ...
                                <div style={{ textAlign: 'right' }}>
                                    <button 
                                        onClick={() => handleViewDetail(item.hn_no)} 
                                        style={{ 
                                            backgroundColor: '#fff', 
                                            color: '#3b62d8', 
                                            border: '1px solid #3b62d8', 
                                            borderRadius: '20px', 
                                            padding: '6px 15px', 
                                            display: 'inline-flex', 
                                            alignItems: 'center', 
                                        ...
                                        onMouseOver={(e) => {
                                            e.currentTarget.style.backgroundColor = '#3b62d8';
...
const SidebarItem = ({ icon, isActive, onClick, themeColor, contentColor }) => {
    return (
        <div onClick={onClick} style={{
            height: '60px', width: '100%', position: 'relative', cursor: 'pointer',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            backgroundColor: isActive ? contentColor : 'transparent',
            borderTopLeftRadius: '30px', borderBottomLeftRadius: '30px',
            marginRight: isActive ? '-1px' : '0', transition: '0.2s'
        }}>
